...
typedef struct
{
  ...
  GeglNode *idref3;
  GeglNode *idref4;
  GeglNode *absentnode1; // Added for auxiliary branches
  GeglNode *absentnode2; // Added for auxiliary branches
  GeglNode *absentnode3; // Added for auxiliary branches
  GeglNode *absentnode4; // Added for auxiliary branches
  GeglNode *behind;
  GeglNode *ripple;
...
static void attach(GeglOperation *operation)
{
  ...
  state->absentnode1 = gegl_node_new_child(gegl, "operation", "gegl:nop", NULL); // Added
  state->absentnode2 = gegl_node_new_child(gegl, "operation", "gegl:nop", NULL); // Added
  state->absentnode3 = gegl_node_new_child(gegl, "operation", "gegl:nop", NULL); // Added
  state->absentnode4 = gegl_node_new_child(gegl, "operation", "gegl:nop", NULL); // Added
}
...
static void update_graph(GeglOperation *operation)
{
  ...
  if (!o->policy) {
    ...
    // Connect absentnode nodes at appropriate points
    gegl_node_link_many(state->input, state->cubism1, state->blur, state->light, state->absentnode1, NULL);
    gegl_node_link_many(state->input, state->cubism1, state->blur, state->light, state->idref1, state->replace, state->absentnode2, NULL);
    gegl_node_link_many(state->input, state->cubism1, state->blur, state->light, state->idref1, state->replace, state->idref2, state->normal1, state->absentnode3, NULL);
    gegl_node_link_many(state->input, state->cubism1, state->blur, state->light, state->idref1, state->replace, state->idref2, state->normal1, state->idref3, state->normal2, state->absentnode4, NULL);
    // Use absentnode nodes for auxiliary branches
    gegl_node_connect(state->replace, "aux", state->color, "output");
    gegl_node_link_many(state->absentnode1, state->ripple, state->medianscale, state->cubism2, state->median1, state->median2, state->color, NULL);
    gegl_node_connect(state->normal1, "aux", state->innerglow, "output");
    gegl_node_link_many(state->absentnode2, state->innerglow, NULL);
    gegl_node_connect(state->normal2, "aux", state->glassoverlay1, "output");
    gegl_node_link_many(state->absentnode3, state->glassoverlay1, NULL);
    gegl_node_connect(state->normal3, "aux", state->glassoverlay2, "output");
    gegl_node_link_many(state->absentnode4, state->glassoverlay2, NULL);
  } else {
